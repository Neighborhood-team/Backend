...
public interface TodayMoodApi {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "오늘의 기분 저장 성공, 성공시 해당 기분의 id값 반환", content = @Content(mediaType = "application/json")),
    ...
    ResponseEntity<Long> saveTodayMood(
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "가족의 기분 조회 성공, 해당 list값을 반환", content = @Content(mediaType = "application/json")),
            @ApiResponse(responseCode = "401", description = "가족의 기분 조회 실패"),
            @ApiResponse(responseCode = "404", description = "Not Found", content = @Content(mediaType = "application/json", schema = @Schema(implementation = ErrorSchema.class))),
            @ApiResponse(responseCode = "500", description = "서버 내부 에러", content = @Content(mediaType = "application/json", schema = @Schema(implementation = ErrorSchema.class)))})
    @GetMapping(produces = {"application/json"})
    ResponseEntity<List<TodayMoodListDto>> getFamilyMoodList(Principal principal);
    ...
    ResponseEntity<Long> updateTodayMood(
